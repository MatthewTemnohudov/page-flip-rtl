...
export class Flip {
    ...
    private checkDirection(direction: FlipDirection): boolean {          
        if (direction === FlipDirection.FORWARD){
            if(this.app.getCurrentPageIndex() < this.app.getPageCount() - 1 === false) {
                this.flipToPage(0, FlipCorner.TOP )
            }
            return this.app.getCurrentPageIndex() < this.app.getPageCount() - 1;
        } else {
            if(this.app.getCurrentPageIndex() >= 1 === false){
                this.flipToPage(this.app.getPageCount() - 1, FlipCorner.TOP )
            }
        }
